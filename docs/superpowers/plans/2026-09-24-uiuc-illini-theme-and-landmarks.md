...
### 15.1 反向赛道（`?reverse=1` 或选单 DIRECTION）

做法：**作者坐标系固定在前向**。`CP_FWD` 是唯一手写的控制点表；`reverse` 只改遍历顺序，其余全部由重建的中线推导（切线、曲率、墙、路缘、banking、AI 线、发车格、龙门）。手写的装饰（加速板/跳台/道具行/14 处地标）用两个辅助函数映射回同一块地面：

```js
const sAt = (sIdx, k = 0) => (((Math.round(sIdx) + (reverse ? -1 : 1) * k) % N) + N) % N;
const lat  = (l) => (reverse ? -l : l);
```

- 跳台是“沿行驶方向长 RAMP_LEN 个采样点”，所以反向时作者点变成它的**远端**（`addRampAt`）。
- 地标按 `(t, side)` 作者化，反向时映射 `t → 1 - t`、`side → -side`。

**踩到的真 bug**：`CP_FWD.slice().reverse()` 会让曲线起点变成 **CP23**，起跑线跑到 t≈1 → 发车格/龙门/圈数全部错位（实测偏移正好是 CP23→CP0 那 92 m ≈ 0.043）。修法是把 CP0 转回队首：`[CP_FWD[0], ...CP_FWD.slice(1).reverse()]`。

**验证**：前向 vs 反向的地标包围盒中心**逐米相同**（stadium `[-55,17.1,-101]`、union `[54,36.7,231]`、morrow `[144,4.5,-374]`）；跳台中心精确镜像（前向 `0.2455/0.3295/0.6515` → 反向 `0.7545/0.6705/0.3485`）；反向自动臀驾驶连跑一圈（进度 1.01→2.47，无敌退、无卡死）。

### 15.2 夜晚模式（`?night=1` 或选单 COURSE）

纯夜晚，全赛道一次切换：

- **天空**：深海军蓝梯度 + **星空**（在 sky shader 里对视线方向做哈希，带微闪）+ 大气雾色变深、距离拉近。
- **月亮**：夜场时满月坐到 `MOON_DIR`（`config.js`）方向上，与**主光源同一方向**（否则影子会和月亮矛盾），半径 60→105，带两层光晕与月海。
- **光照**：方向光→月光（`0x9dbfe8`、0.85），半球光变暗变蓝（0.7）+ 暖地面反弹；环境反射强度 0.55→0.3。
- **可读性（关键）**：沥青本身保持暗，但路缘、护栏面板、桥栏、跳台条纹、以及沥青贴图里烘的路面白线全部走 `nightGlow()` → `emissiveMap`，像反光漆一样发光。灯笼串/月饼摊/加速板/道具箱本来就是不吃光的材质 + bloom，夜里自然变成光源。
- 云从“白天亮白”改为深夜蓝（`color/emissive` 都压下去）。

### 15.3 CI 扩展

`tools/verify.cjs` 现在跑 **两条赛道 × 全部不变量**（中线自洽、AI 线不出路面、地形不穿路面、八个发车格在路面上），加上：

- 反向必须真的生效（`track.reverse` + 名字后缀）；
